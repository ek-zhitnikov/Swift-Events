...
//        }
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        //логин
        if indexPath.row == 2 {
            guard let login = textLogin.text, !login.isEmpty, let password = textPassword.text, !password.isEmpty else {
                print("ошибка ввода")
                return
            }
        ...
        if indexPath.row == 3 {
            performSegue(withIdentifier: "goToSingUpControllerSegueID", sender: self)
        }
    }
